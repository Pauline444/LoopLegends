...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function createHomePageContent() {
    ...
    if (interactiveContainer) {
      ...
      const secondPart = document.createTextNode(
        " är ett modernt nätverk för dig som vill växa - både personligt och professionellt. Upptäck inspirerande individer, se hur de strukturerar sina dagar och ta del av deras bästa metoder för fokus, balans och framgång."
      );
  ...
  window.addEventListener("load", () => {
    ...
    if (
      ...
      (!interactiveContainer.children.length ||
        !interactiveContainer.innerHTML.trim())
    ) {
      createHomePageContent();
    }
  });
...
const links = document.querySelectorAll(".nav-tab a");
